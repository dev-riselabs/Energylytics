      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
        {
          navLinks.map(linkItem => <div key={linkItem.title} className="flex flex-col gap-4 md:gap-5">
            ...
            <div className="flex flex-col gap-3 md:gap-4 list-disc">
              {
                linkItem.links.map(link => <Link key={link.title} to={link.path} className="text-xs sm:text-sm md:text-base text-slate hover:text-green25 transition-all font-light flex items-start sm:items-center gap-1.5"> <GoDotFill className="w-3 h-3 shrink-0 mt-1 sm:mt-0" />{link.title}</Link>)
              }

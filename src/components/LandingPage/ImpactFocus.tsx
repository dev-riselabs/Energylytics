  ...
  return (
    <section className="flex flex-col gap-6 md:gap-7 items-center font-inter bg-green25 px-5 sm:px-7 md:px-10 lg:px-15 py-12 md:py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        <img src="/impact-focus.jpg" alt="" className="w-full md:col-span-2 h-64 sm:h-80 md:h-full max-h-[500px] object-cover rounded-2xl md:rounded-3xl"/>
        <div className="flex flex-col gap-4 md:col-span-1 md:p-7.5 md:justify-center">
          {chains.map(({ icon: Icon, title }) => (
            <div key={title} className="flex items-center gap-4">
              <div className="w-14 h-14 md:w-18.5 md:h-18.5 rounded-md flex items-center justify-center bg-green45 shrink-0">
                <Icon className="w-5 h-5 md:w-6 md:h-6 text-green25" />
              </div>
              <h5 className="text-base sm:text-lg md:text-xl text-white max-w-[20ch] md:max-w-[15ch]">{title}</h5>
            </div>
          ))}
        </div>
      </div>

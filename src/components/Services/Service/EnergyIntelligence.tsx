    ...
    <section id='energy-analytics' className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-10 px-5 sm:px-7 md:px-15 bg-zinc800 py-15">
      <div className="flex flex-col gap-7">
        ...
        <div className="flex flex-col gap-4">
            {
                energyStats.map(stat => <span className="text-base text-slate50 flex items-center gap-2"><FaCircleCheck className="w-5 h-5 text-green25" /> {stat}</span>)
            }
        </div>
      </div>
      <img src="/energy-intelligence-img.png" alt="" className="rounded-4xl h-full" />
      <div className="border border-zinc rounded-3xl p-10 flex flex-col gap-6 bg-green55">
        <h4 className="text-xl font-manrope text-slate font-medium">Energy Data Analytics</h4>
        <div className="flex flex-col gap-4">
            {
                dataStats.map(stat => <span className="text-base text-slate50 flex items-center gap-2"><FaCircleCheck className="w-5 h-5 text-green25" /> {stat}</span>)
            }
        </div>
      </div>
      <div className="border border-zinc rounded-3xl p-10 flex flex-col gap-6 bg-green55">
        <h4 className="text-xl font-manrope text-slate font-medium">Energy Decision Intelligence</h4>
        <p className="text-base text-slate60">We help our users answer the following questions:</p>
        <div className="flex flex-col gap-4">
            {
                decisionStats.map(stat => <span className="text-base text-slate50 flex items-center gap-2"><FaCircleCheck className="w-5 h-5 text-green25" /> {stat}</span>)
            }
        </div>
      </div>
    </section>

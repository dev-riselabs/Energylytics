...
function SmartManagement() {
  ...
  return (
    <section className="bg-white px-5 sm:px-7 md:px-15 py-15 grid grid-cols-1 md:grid-cols-2 gap-6 font-inter">
      <div className="flex flex-col gap-6.5 justify-center">
        ...
        <div className="flex flex-col gap-4">
            {
                faqs.map(({question, answer}, i) => <div key={question} className="bg-zinc600 border border-green25 p-5 sm:p-7.5 flex  gap-3.5 rounded-2xl">
                    ...
                    <div className="flex flex-col gap-6 flex-1">
                        ...
                        {activeFaq === i + 1 && <p className="text-sm text-slate50">{answer}</p>}
                    </div>
      ...
      <img
        src="/raw-data.png"
        alt=""
        className="  rounded-3xl sm:h-auto min-h-80 w-full object-scale-down sm:object-fill"
      />
    </section>
  );
}

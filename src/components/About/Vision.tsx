...
function Vision() {
  return (
    <section
      id="about-mission"
      className="home-about-bg flex flex-col gap-6 px-5 sm:px-7 md:px-10 lg:px-15 py-12 md:py-20"
    >
      <div className="flex flex-col gap-4 md:gap-5">
        <div className="flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-2 rounded-full border border-green50 self-center">
          ...
          <span className="text-green25 text-xs sm:text-sm font-bold">
            About Us
          </span>
        </div>
        ...
        <h4 className="text-base sm:text-lg md:text-xl text-center text-slate ">
          ...
          environmental intelligence, project management and knowledge systems
          to help individuals, governments, businesses and development partners
          turn energy data into better decisions and better decisions into
          better energy infrastructure along the clean energy value chain.
        </h4>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {about.map(({ title, description, icon: Icon, img }, i) => (
          <div
            key={title}
            className="bg-green35 p-6 sm:p-8 md:p-10 rounded-[20px] gap-5 md:pr-0 grid grid-cols-1 sm:grid-cols-2 place-items-center"
          >
            <div className="flex flex-col gap-5 md:gap-7 ">
              ...
              <div className="flex flex-col gap-4 md:gap-6">
                <h4 className="border-b border-green60 pb-2 text-base font-bold text-slate60 self-start">
                  {title}
                </h4>
            ...
            <img
              src={img}
              alt=""
              className={`w-full h-full object-cover max-h-60 md:max-h-70 ${i === 1 ? "rounded-3xl md:skew-y-14" : "rounded-full"}`}
            />
          </div>
        ))}
      </div>

...
function Header() {
  ...
  return (
    <header className="flex items-center justify-between bg-white h-20 lg:gap-8 lg:h-30.5 px-5 sm:px-7 md:px-15">
      <Link to="/" className="cursor-pointer">
        <img
          src="/logo.png"
          alt=""
          className="w-24 sm:w-32 md:w-40 lg:w-50 xl:w-60 h-12 sm:h-16 lg:h-25 object-contain"
        />
      </Link>
      <button
      ...
      <div className="lg:flex items-center gap-5 hidden xl:flex-1 xl:justify-between ">
        ...
        <div className="flex flex-col lg:flex-row gap-3">
          <button className="px-2 xl:px-5 py-3 rounded-xl border border-green30 text-green text-[10px] xl:text-sm font-bold font-dmSans hover:bg-green25 hover:text-white transition-all cursor-pointer">
            Energy Calculator
          </button>
          <button className="px-2 xl:px-5 py-3 rounded-xl border border-green bg-green25 text-white text-[10px] xl:text-sm font-bold font-dmSans hover:bg-transparent hover:text-green transition-all cursor-pointer">
            Login/Sign Up
          </button>
        </div>
      </div>
   ...
   `}
      >
        <nav className="flex flex-col gap-4 xl:gap-4">
          {navLinks.map((navItem) => (
            <div
              key={navItem.title}
              className="relative"
                ...
                className={({ isActive }) =>
                  `${
                    isActive && !navItem.children
                      ? "text-green25 font-bold hover:text-green"
                      : "text-zinc100 hover:text-slate"
                  } transition-all text-base capitalize flex items-center gap-1`
                }
              >
                <MdPlayArrow className="w-4 h-4" />
